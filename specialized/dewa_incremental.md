# DEWA Incremental Usage Scraper

This is a web scraper that extracts **billing-period** electricity and water consumption totals
from the DEWA (Dubai Electricity and Water Authority) website using Playwright. For per-day
totals instead, see [dewa_daily.md](dewa_daily.md).
...
podman run --rm --name mini-scraper \
  -v $(PWD)/dewa_incremental.js:/scraper/scraper.js \
  -v $(PWD)/logs:/logs \
...
- `PERIOD` - The period to retrieve data for (default: 'CURRENT'). Can be 'CURRENT' or a date in
  yyyy-mm format (e.g., 2023-12). For 'CURRENT', the scraper identifies the right dropdown option
  on the usage page by checking, in order: an option with value `UnbilledConsumption`, then an
  option whose label contains "till yesterday" (case-insensitive), then the first option in the
  list if its value represents the current or next calendar month. If none of these match, the
  scraper fails with an error rather than guessing. When a specific yyyy-mm is given, it must be
  May 2025 or later and must not be the current (or a future) month
...
### Success Output
When the scraper runs successfully, it outputs a JSON object to stdout with the following
structure:
```json
{
  "period": "2026-06-22T00:00:00+04:00",
  "electricity": "2221.88",
  "water": "12.09"
}
```

`period` is the ISO 8601 start date/time (Asia/Dubai, UTC+4) of the billing period the totals
cover — DEWA billing periods run from the 22nd of one calendar month to the 21st of the next.
For a specific `PERIOD=yyyy-mm`, this is the 22nd of the *previous* calendar month; for
`PERIOD=CURRENT` resolving to the unbilled period, it's the 22nd of the dropdown's selected
month. The electricity consumption is measured in **kilowatt-hours (kWh)** and water consumption
is measured in **cubic meters (m³)**.
...
These files are automatically generated and stored in the logs directory that is mounted to
...
3. If you see `Error: Unable to determine the current period from available options`, DEWA has
   likely changed the period dropdown's option values or labels again. Enable
   `CONSOLE_LOG=debug` or `FILE_LOG=debug` and re-run with `PERIOD=CURRENT` to see the actual
   `Available periods` list logged, then update the matching logic in `dewa_incremental.js`
   accordingly.
4. If you see `Error: Requested period '<value>' is not available`, the period dropdown's value
   format may have changed; check the logged `Available periods` list to confirm the expected
   format (currently `mmyyyy`, e.g. `072025` for July 2025).
5. If you see `Error: Unable to determine billing period for UnbilledConsumption` or `Error:
   Unable to parse period value '<value>'`, DEWA changed the shape of the period dropdown's
   `value` attribute (currently expected as `mmyyyy`, parsed by `parsePeriodValue()` in
   `dewa_incremental.js`) or the dropdown no longer lists an entry immediately after
   `UnbilledConsumption`. Check the logged `Available periods` list.
